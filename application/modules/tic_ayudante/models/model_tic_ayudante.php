...
class Model_tic_ayudante extends CI_Model
{
	...
	function __construct()
	{
        ...
        var $table = 'tic_ayudante_orden'; //El nombre de la tabla que estamos usando
	...
	public function ayudantes_enOrden($id_orden_trabajo)//retorna un entero de cantidad de ayudantes asignados en una orden
	{
		...
		$query=$this->db->get_where('tic_ayudante_orden', $aux)->num_rows;
		return($query);
	}
	...
	{
		$this->db->select('*');
		$this->db->from('tic_ayudante_orden');
		$this->db->where('id_cuadrilla', $num_cuadrilla);
		$this->db->where('id_orden_trabajo', $num_sol);
		$this->db->join('tic_miembros_cuadrilla', 'tic_miembros_cuadrilla.id_trabajador=tic_ayudante_orden.id_trabajador');
	...
	public function ayudante_a_orden($array)//asigna un ayudante a una orden, el array debe contener los nombres de las columnas como keys, y los datos
	{
		if(!empty($array))
		{
			...
			$this->db->insert('tic_ayudante_orden', $array);
			return(TRUE);
...
//                    die_pre($array);
			...
			$this->db->where($array);
			$this->db->delete('tic_ayudante_orden');
			return(TRUE);
                    endif;
		}
		else
	...
	public function ayudante_en_orden($id_trabajador, $id_orden_trabajo)//verifica si un ayudante esta en la orden
	{
		...
		$query = $this->db->get_where('tic_ayudante_orden', $array);
		if($query->num_rows==0)
		{
			return(FALSE);
		}
...
//                $this->db->or_where('cargo', 'Jefe de Mantenimiento');
		...
		$this->db->from('dec_usuario');
            ...
            $this->db->join('tic_ayudante_orden', 'tic_ayudante_orden.id_trabajador = dec_usuario.id_usuario','right');
            // die_pre($this->db->get()->result_array(), __LINE__, __FILE__);
            return($this->db->get()->result_array());
	}
	public function ayudantes_NoDeOrden($id_orden_trabajo)//lista los ayudantes que no esten asignados a una orden (sujeto a ser mejorado)
	{
		...
		$query=$this->db->get_where('tic_ayudante_orden', $aux);
...
//                        $this->db->or_where('cargo', 'Jefe de Mantenimiento');
	...
	public function ordenes_y_ayudantes()//lista completamente todos los datos de la tabla de tic ayudante orden
	{
		// $this->db->order_by("id_trabajador", "desc");
		return($this->db->get('tic_ayudante_orden')->result_array());
	}
	...
	$this->db->where('id_tipo', $id_tipo);
        $this->db->join('tic_orden_trabajo', 'tic_orden_trabajo.id_orden=tic_ayudante_orden.id_orden_trabajo');
        $this->db->join('dec_usuario', 'dec_usuario.id_usuario=tic_ayudante_orden.id_trabajador');
        $this->db->order_by("nombre", "asc"); 
        $this->db->group_by("tic_ayudante_orden.id_trabajador");
//	 die_pre($this->db->get()->result_array(), __LINE__, __FILE__);
	return($this->db->get()->result_array());
    }
...
//        $filtro = "WHERE estatus NOT IN (3,4)";
          $sJoin = " INNER JOIN tic_orden_trabajo ON tic_orden_trabajo.id_orden=tic_ayudante_orden.id_orden_trabajo "
                . "INNER JOIN dec_dependencia ON tic_orden_trabajo.dependencia=dec_dependencia.id_dependencia "
                . "INNER JOIN mnt_estatus ON tic_orden_trabajo.estatus=mnt_estatus.id_estado "
                . "INNER JOIN tic_tipo_orden ON tic_orden_trabajo.id_tipo=tic_tipo_orden.id_tipo "
                . "INNER JOIN dec_usuario ON dec_usuario.id_usuario=tic_ayudante_orden.id_trabajador "; 
