...
class Tic_ayudante extends MX_Controller {

    function __construct() { //constructor predeterminado del controlador
        parent::__construct();
        $this->load->library('form_validation');
        $this->load->library('pagination');
        $this->load->model('model_tic_ayudante');
        ...
        $this->load->model('tic_asigna_cuadrilla/model_tic_asigna_cuadrilla');
...
//            die_pre($_POST);
            ...
            if (!empty($_POST['responsable'])) {
                ...
                if (!($this->model_responsable->existe_resp(array('id_orden_trabajo' => $_POST['id_orden_trabajo'])))) {
                    ...
                    $insert = array(
                        ...
                        $this->model_tic_estatus_orden->insert_orden($insert);
                } else {
                    ...
                    $this->model_tic_ayudante->ayudante_fuera_deOrden(array('id_trabajador' => $responsable['id_responsable'], 'id_orden_trabajo' => $_POST['id_orden_trabajo']));
                };
                if (!$this->model_tic_ayudante->ayudante_en_orden($_POST['responsable'], $_POST['id_orden_trabajo'])) {
                    $a = 1;
                    $guardar2 = array(
                        'id_trabajador' => $_POST['responsable'],
                        'id_orden_trabajo' => $_POST['id_orden_trabajo']);
                    $asignados = $asignados + $this->model_tic_ayudante->ayudante_a_orden($guardar2);
                }
            };
            ...
            if (isset($_POST['cut'])){
                ...
                $this->model_tic_ayudante->ayudante_fuera_deOrden(array('id_trabajador' => $responsable['id_responsable'], 'id_orden_trabajo' => $_POST['id_orden_trabajo']));
                $this->model_tic_ayudante->ayudante_fuera_deOrden(array('id_orden_trabajo' => $_POST['id_orden_trabajo']));
                $update = array(
                    'fecha' => $fecha,
                    'estatus' => 1);
                $this->model_tic_solicitudes->actualizar_orden($update, $num_sol);
                $insert = array(
                    'id_estado' => 1,
                    'id_orden_trabajo' => $num_sol,
                    'id_usuario' => $this->session->userdata('user')['id_usuario'],
            ...
            while (sizeof($_POST) > 1) {//pasa el arreglo del post completo, para luego separar los id's de los trabajadores a asignar, y los trabajadores a remover de la orden
                if (array_key_exists('assign' . $i, $_POST)) {//aqui agarra los que van a ser asignados
                    ...
                    if (!$this->model_tic_ayudante->ayudante_en_orden($aux['id_trabajador'], $aux['id_orden_trabajo'])) {
                        $asignados = $asignados + $this->model_tic_ayudante->ayudante_a_orden($aux);
                ...
                if (array_key_exists('remove' . $i, $_POST)) {//aqui agarra los que van a ser removidos de la orden
                    ...
                    // echo_pre($aux, __LINE__, __FILE__);
                    $removidos = $removidos + $this->model_tic_ayudante->ayudante_fuera_deOrden($aux);
...
////////////////////////opcional
            if (array_key_exists('id_trabajador', $_POST)) {//por si se quiere asignar un ayudante desde otra vista, y solo 1
                if (!$this->model_tic_ayudante->ayudante_en_orden($_POST['id_trabajador'], $_POST['id_orden_trabajo'])) {
                    $a = 1;
                    $asignados = $asignados + $this->model_tic_ayudante->ayudante_a_orden($_POST);
                }
            }
/////////////////////////fin de opcional
            ...
            if ($a > 0) {
                if ($asignados) {
                    //actualizar en tic_solicitudes
                    //fecha (fecha de timestamp), y estatus (2 correspondiente a "EN_PROCESO")
                    $update = array(
                        'fecha' => $fecha,
                        'estatus' => 2);
                    $this->model_tic_solicitudes->actualizar_orden($update, $num_sol);
                    //guardar en tic_estatus_orden con valores de:
            ...
            if ($r > 0) {
                if ($removidos) {
                    $cuadrilla = $this->model_tic_asigna_cuadrilla->tiene_cuadrilla(intval($num_sol));
                    if ($cuadrilla) {//si tiene una cuadrilla asignada
                        $aux = $this->model_tic_ayudante->ayudantesDeCuadrilla_enOrden($num_sol, $cuadrilla);
                        if (empty($aux)) {
                            $array = array('id_ordenes' => intval($num_sol), 'id_cuadrilla' => $cuadrilla);
                            $this->quitar_cuadrilla($array);
                        }
                    }
                    if ($this->model_tic_ayudante->ayudantes_enOrden($num_sol) == 0) {
                        $update = array(
                            'fecha' => $fecha,
                            'estatus' => 1);
                        $this->model_tic_solicitudes->actualizar_orden($update, $num_sol);
    ...
    public function assigned($id_orden_trabajo) {
        ...
        // die_pre($this->model_tic_ayudante->ayudantes_DeOrden($id_orden_trabajo), __LINE__, __FILE__);
        return($this->model_tic_ayudante->ayudantes_DeOrden($id_orden_trabajo));
    }

    public function unassigned($id_orden_trabajo) {
        // echo_pre($id_orden_trabajo, __LINE__, __FILE__);
        // die_pre($this->model_tic_ayudante->ayudantes_NoDeOrden($id_orden_trabajo), __LINE__, __FILE__);
        return($this->model_tic_ayudante->ayudantes_NoDeOrden($id_orden_trabajo));
    }
    ...
    public function mostrar_assigned() {
        if ($this->input->post('id')):
            ...
            public function quitar_cuadrilla($array) {//cut, cuadrilla
                $this->model_tic_asigna_cuadrilla->quitar_cuadrilla($array); //quita la asignacion de la cuadrilla
                $this->session->set_flashdata('asigna_cuadrilla', 'quitar');
            }

            /* End of file tic_ayudante.php */
            /* Location: ./application/modules/tic_ayudante/controllers/tic_ayudante.php */

            public function reportes() {
                $results = $this->model_tic_ayudante->get_list(); //Va al modelo para tomar los datos para llenar el datatable
...
//            echo_pre($view['estatus']);
                    ...
                    $this->load->view('tic_ayudante/reporte_trabajador', $view);
...
//                echo $this->model_tic_ayudante->consul_trabaja_sol($all['id_usuario'],$this->input->post('estatus'),$this->input->post('fecha1'),$this->input->post('fecha2'),$band);
                if ($this->model_tic_ayudante->consul_trabaja_sol($all['id_usuario'], $this->input->post('estatus'), $this->input->post('fecha1'), $this->input->post('fecha2'), $band)):
                    $cont++;
                    ?>
    ...
    public function load_consult() {
        ...
        if (is_numeric($this->input->post('id_trabajador'))):
            $datos = $this->model_tic_ayudante->consul_trabaja_sol($this->input->post('id_trabajador'), $this->input->post('estatus'), $this->input->post('fecha1'), $this->input->post('fecha2'), $band);
        ...
        elseif ($this->input->post('id_trabajador') == 'all'):
            $datos = $this->model_tic_ayudante->consul_trabaja_sol('', $this->input->post('estatus'), $this->input->post('fecha1'), $this->input->post('fecha2'), $band);
//            die_pre($datos);
            ?>
                <style>
                    tr.group,
            ...
            //                    "ajax": {
            //                        "url": "<?php echo site_url('tic_ayudante/tic_ayudante/reportes') ?>",
            //                        "type": "GET",
            //                        "data": function ( d ) {
            //                            d.uno = $('#result1').val();
            //                            d.dos = $('#result2').val();
            ...
            public function pdf_reportes_worker() {//aqui estoy haciendo los reportes
                ...
                if (($_POST['tipo']) == 'trabajador'):
                    if (isset($_POST['id_trabajador'])):
                        $view['tabla'] = $this->model_tic_ayudante->consul_trabaja_sol($_POST['id_trabajador'], $_POST['estatus'], $_POST['fecha1'], $_POST['fecha2'], $band); //construccion de la tabla
                        $view['trabajador'] = $this->model_user->get_user_cuadrilla($_POST['id_trabajador']);
//            echo_pre($view);
                    else:
                        $view['tabla'] = $this->model_tic_ayudante->consul_trabaja_sol('', $_POST['estatus'], $_POST['fecha1'], $_POST['fecha2'], $band); //construccion de la tabla
                    endif;
                endif;
                if (($_POST['tipo']) == 'responsable'):
                    if (isset($_POST['id_trabajador'])):
                        ...
                        foreach ($view['tabla'] as $dat):
                            $ayudantes[$dat['id_orden']] = $this->model_tic_ayudante->ayudantes_DeOrden($dat['id_orden']);
                        endforeach;
...
//            echo_pre($view);
                ...
                if (($_POST['tipo']) == 'tipo_orden'):
                    if (isset($_POST['id_cuad'])):
                        $view['tabla'] = $this->model_tic_asigna_cuadrilla->consul_cuad_sol($_POST['id_cuad'], $_POST['estatus'], $_POST['fecha1'], $_POST['fecha2'], $band);
                        $view['cuadrilla'] = $this->model_tic_cuadrilla->get_nombre_cuadrilla($_POST['id_cuad']);
                        foreach ($view['tabla'] as $dat):
                            $ayudantes[$dat['id_orden']] = $this->model_tic_ayudante->ayudantes_DeOrden($dat['id_orden']);
                        endforeach;
                        $view['ayudantes'] = $ayudantes;
//            echo_pre($view);
                        ...
                        $view['tabla'] = $this->model_tic_asigna_cuadrilla->consul_cuad_sol('', $_POST['estatus'], $_POST['fecha1'], $_POST['fecha2'], $band);
                        foreach ($view['tabla'] as $dat):
                            $ayudantes[$dat['id_orden']] = $this->model_tic_ayudante->ayudantes_DeOrden($dat['id_orden']);
                        endforeach;
                        $view['ayudantes'] = $ayudantes;
