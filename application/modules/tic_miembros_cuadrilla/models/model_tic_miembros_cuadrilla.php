...
class Model_tic_miembros_cuadrilla extends CI_Model {
    ...
    public function get_miembros() {
        return $this->db->get('tic_miembros_cuadrilla')->result();
    }
    ...
    public function guardar_miembros($datos = '') {
        if (!empty($datos)) { //verifica que no se haga una insercion vacia
         if (!$this->existe_cuad($datos)):
            $this->db->insert('tic_miembros_cuadrilla', $datos);
    ...
    {
        $this->db->where($datos);
        if($this->db->count_all_results('tic_miembros_cuadrilla') > 0):
            return TRUE;
        endif;
        return FALSE;
    }
    ...
    public function borrar_by_id($id='',$cuad='')
	{
          $this->load->model('tic_cuadrilla/model_tic_cuadrilla');
            if (!$this->model_tic_cuadrilla->es_responsable($id,$cuad)):
              $this->db->where('id_cuadrilla', $cuad);
              $this->db->where('id_trabajador', $id);
	      $this->db->delete('tic_miembros_cuadrilla');
              return TRUE;
           else:
               return FALSE;
              endif;
