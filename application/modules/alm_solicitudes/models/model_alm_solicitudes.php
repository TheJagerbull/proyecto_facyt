...
//// FIN DE CONSULTAS DE ADMINISTRADOR DE SOLICITUDES
	...
	{/////en uso
		...
		{
			$this->db->select('id_articulo, descripcion, estado_articulo AS estado, motivo_alm, cant_solicitada AS cant, cant_aprobada AS cant_aprob, cant_usados, cant_nuevos, unidad, reserv, nuevos + usados AS disp');
			$this->db->join('alm_articulo', 'alm_articulo.ID=alm_art_en_solicitud.id_articulo');
			// $this->db->join('alm_art_en_solicitud', 'alm_art_en_solicitud.id_articulo=alm_articulo.ID AND alm_art_en_solicitud.estado_articulo="activo" OR alm_art_en_solicitud.motivo_alm!=""');
...
////FIN de Para desactivar un articulo por motivo de administrador de almacen
	...
	public function libera_art($array)
	{
		...
		foreach ($solicitud as $key => $value)//para recorrer los renglones de articulos de la solicitud
		{
			...
			if($query['estado_articulo']=='activo')//valida que el articulo no halla sido cancelado de la solicitud
			{
				$aprob = $query['cant_aprobada'];
				$nuevos = $query['cant_nuevos'];
				$usados = $query['cant_usados'];

				$art['ID'] = $value['id_articulo'];
				$this->db->where($art);
				$query = $this->db->get('alm_articulo')->row_array();//consulto el articulo en sistema que conside con el de la solicitud

				$query['reserv'] = ($query['reserv'] - ($aprob));//disminuyo de reservados
				//entonces la cantidad reservada de ese articulo, pasa a ser la resta de la cantidad reservada anterior, menos la resta entre la cantidad aprobada anterior menos la cantidad aprobada actual

				$query['nuevos'] = ($query['nuevos'] + ($nuevos));//se lo sumo a articulos nuevos si esos eran los que se reservaron antes

				$query['usados'] = ($query['usados'] + ($usados));//se lo sumo a articulos usados si esos eran los que se reservaron antes
