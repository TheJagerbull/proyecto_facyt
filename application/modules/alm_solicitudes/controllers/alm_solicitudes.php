...
////////fin de agregar y quitar articulos de la session
	public function eliminar_solicitud()//elimina el carrito de articulos (solicitud sin enviar)
	{
...
//     }
    public function editar_solicitud()
    {
        if($this->session->userdata('user'))
        {
            if($this->dec_permiso->has_permission('alm', 11))
            {
                $this->load->view('template/header');
                $this->load->view('alm_solicitudes/editor_solicitud');
                $this->load->view('template/footer');
            }
            else
            {
    ...
    public function enviar_solicitud()//completada (de carrito a solicitud de almacen)
    {
//    	echo_pre('permiso para enviar solicitudes', __LINE__, __FILE__);//14
	    if($this->session->userdata('user') && ($this->dec_permiso->has_permission('alm', 14) || $this->dec_permiso->has_permission('alm', 9)))
	    {
	    	...
	    	if($_POST && $this->dec_permiso->has_permission('alm', 14))//recibe el formulario, Y debe terner el permiso para envios
	    	{
	    		...
	    		// echo_pre($_POST, __LINE__, __FILE__);
	    		// if($this->change_statusSol($_POST))
	    		if($this->model_alm_solicitudes->insert_solicitud($_POST['send']['id_carrito']))
	    		{
    				$this->updateUserCart();
	    			$this->session->set_flashdata('send_solicitud', 'success');
	    			redirect($uri);
...
//    	echo_pre('permiso para aprobar solicitudes', __LINE__, __FILE__);//12
    ...
    public function anular()
    {
    	if($this->session->userdata('user') && $this->dec_permiso->has_permission('alm', 14))
    	{
            ...
            {
                // die_pre($_POST, __LINE__, __FILE__);
                if($this->model_alm_solicitudes->anular_solicitud($_POST))
                {
    ...
    public function cancelar_carrito()
    {
        if($this->session->userdata('user'))
        {
            if($this->input->post())
            {
                ...
                $uri = $post['uri'];
                unset($post['uri']);
                if($this->model_alm_solicitudes->delete_carrito($post))
                {
                    $this->updateUserCart();
                    redirect($uri);
                }
								...
								$("button[id^=\'an\']").on("click", function(){
									...
									else
									{
										...
										$("#anula"+aux).submit();
                                                                ...
                                                                $("button[id^=\'in\']").on("click", function(){
									aux=this.id.slice(2);
									console.log(aux);
									var input = $("#cancela"+aux+" textarea").val();
									var msg = $("#cancela"+aux+" #motivo_msg");
									console.log(msg);
									if(input.length===0)
									{
										console.log("error");
...
//////////////////////modal de revisar///
                    ...
                    {
                            if($this->session->userdata('id_carrito')==$refID)//debo preguntar si es propietario de la solicitud
                            {
    //////////////////////modal de enviar///
                                $auxEnlaces .='<a href="#enviar'.$refID.'" data-toggle="modal" title="Envia la solicitud"><i class="glyphicon glyphicon-share color"></i></a>';
                                $auxModales .='<div id="enviar'.$refID.'" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                                      <div class="modal-dialog">
                                        <div class="modal-content">
                                          <div class="modal-header">
                                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">X</button>
                                          ...
                                          <div class="modal-body">
                                            ...
                                            <div class="form-group">
                                                <input form="send'.$refID.'" hidden name="send[id_carrito]" value="'.$refID.'"/>
                                                <input form="send'.$refID.'" hidden name="send[uri]" value="solicitudes/usuario"/>
                                            </div>
                                          </div>
                                          <div class="modal-footer">
                                                <button type="button" class="btn btn-default" data-dismiss="modal" aria-hidden="true">Cerrar</button>
                                                <button form="send'.$refID.'" type="submit" class="btn btn-primary">Continuar</button>
                                          </div>
                                          </form>
                                          </div>
    ...
    //////////////////////Fin de modal de enviar///
        ...
        //////////////////////Fin de modal de revisar///
                                  ...
                                  <div class="modal-dialog">
                                    <div class="modal-content">
                                      ...
                                      <div class="modal-body">
                                        ...
                                        <div class="form-group">
                                            <input form="delete'.$refID.'" hidden name="cancelar[id_carrito]" value="'.$refID.'"/>
                                            <input form="delete'.$refID.'" hidden name="cancelar[uri]" value="solicitudes/departamento"/>
                                        </div>
                                      </div>
                                      <div class="modal-footer">
                                            <button type="button" class="btn btn-default" data-dismiss="modal" aria-hidden="true">Cerrar</button>
                                            <button form="delete'.$refID.'" type="submit" class="btn btn-primary">Continuar</button>
                                      </div>
                                      </form>
                                      </div>
    				    ...
    				    // $auxEnlaces .= '<a title="cancela la solicitud" onClick="cancelar(\'id_carrito\');" ><i class="glyphicon glyphicon-remove"></i></a>';
                    }
    			break;
                              ...
                              <div class="modal-dialog">
                                <div class="modal-content">
                                  ...
                                  <div class="modal-body">
                                    ...
                                    <div class="form-group">
                                        <input form="delete'.$refID.'" hidden name="cancelar[id_carrito]" value="'.$refID.'"/>
                                        <input form="delete'.$refID.'" hidden name="cancelar[uri]" value="solicitudes/usuario"/>
                                    </div>
                                  </div>
                                  <div class="modal-footer">
                                        <button type="button" class="btn btn-default" data-dismiss="modal" aria-hidden="true">Cerrar</button>
...
/////////////////////Fin de Modal de Anular solicitud///
					...
					// if($this->dec_permiso->has_permission('alm', 16)&&($sol_status=='en_proceso'))
					// {
                                ...
                                <div class="modal-content">
                                  ...
                                  <div class="modal-body">                   
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input hidden name="cancelar[uri]" value="solicitudes/departamento"/>
                                        <br>
                                        <br>
                                        <br>
                                            </div>
                                            ...
                                            <div class="modal-content">
                                              ...
                                              <div class="modal-body">                   
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <input hidden name="cancelar[nr_solicitud]" value="'.$refID.'"/>
                                                    <input hidden name="cancelar[uri]" value="solicitudes/usuario"/>
                                                    <br>
                                                    <br>
                                                    <br>
                                                        </div>
                                            ...
                                            <div class="modal-content">
                                              ...
                                              <div class="modal-body">                    
                                                ...
                                                <div class="table-responsive">
                                                    ...
                                                    <input hidden name="completa[nr_solicitud]" value="'.$refID.'"/>
                                                    <input hidden name="completa[uri]" value="solicitudes/usuario"/>
                                                        <div class="col-lg-12"><br></div>
                                                        <div class="modal-footer">
                                                            <button type="button" class="btn btn-default" data-dismiss="modal" aria-hidden="true">Cerrar</button>
                                                            <button id="com'.$refID.'" form="cancela'.$refID.'" type="button" class="btn btn-primary">Guardar</button>
                                                    ...
                                                    foreach ($hist as $key => $record)
                                                    {
                                                        ...
                                                        $auxModales.='<tr>';
                                                                switch ($record['status_ej'])
                                                                {
                                                                    ...
                                                                    break;
                                                                    case 'enviado':
                                                                        $auxModales.= '<td><span class="label label-success">Despach&oacute; articulos</span></td>';//Estado actual
                                                                    break;
                                                                    case 'retirado':
                                                                        $auxModales.= '<td><span class="label label-info">Retir&oacute; articulos</span></td>';//Estado actual
                                                                    break;
                                                                    case 'completado':
