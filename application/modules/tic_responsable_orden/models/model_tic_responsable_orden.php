...
class Model_tic_responsable_orden extends CI_Model {
    ...
    public function set_resp($data = '') { //funcion para guardar el responsable de una solicitud
        if (!empty($data)) {
            ...
            //die_pre($data4);
                $this->db->insert('tic_responsable_orden', $data);
                return TRUE;
            endif;
        }
        return FALSE;
    ...
    {
        $this->db->join('tic_orden_trabajo', 'tic_orden_trabajo.id_orden = tic_responsable_orden.id_orden_trabajo', 'INNER');
    ...
    public function existe_resp_2($id_usuario='',$status='',$fecha1='',$fecha2='',$id_tipo='') //funcion para verificar si existe un responsable en la solicitud con fechas
    {
        ...
        $query = $this->db->get('tic_responsable_orden')->result();
        if (!empty($query)):
            return TRUE;
        else:
            return FALSE;
    ...
    function edit_resp($data = '',$resp_orden=''){ //funcion para editar el responsable de la solicitud
        $this->db->where($data);
        $this->db->update('tic_responsable_orden',array('id_responsable' => $resp_orden));
    }

    function del_resp($sol=''){ //funcion para eliminar un responsable
        $this->db->delete('tic_responsable_orden',array('id_orden_trabajo' => $sol));
    }
    ...
    public function es_respon_orden($respon_orden='',$sol=''){ //funcion para verificar si es reponsable de una orden
        ...
        $query = $this->db->get_where('tic_responsable_orden',$datos);
        if($query->num_rows() > 0)
            return TRUE;
