...
class Model_tic_reporte extends CI_Model
{
    ...
    function get_list(){
        $table = 'tic_orden_trabajo'; //El nombre de la tabla que estamos usando
        $ayuEnSol = $this->model_tic_ayudante->array_of_orders(); //Para consultar los ayudantes asignados a una orden
        $cuadri = $this->model_tic_cuadrilla->get_cuadrillas();
...
//        es para usarla en el query que se va a construir mas adelante. Este datos es modificable */
        $id_tipo = $this->model_tic_cuadrilla->es_resp_no_jefe_cuad($this->session->userdata('user')['id_usuario']);
        if(!empty($id_tipo)):
            if (isset($filtro)):
...
//                    . "LEFT JOIN tic_estatus_orden ON tic_estatus.id_estado=tic_estatus_orden.id_estado "
...
//                . "LEFT JOIN tic_responsable_orden ON tic_orden_trabajo.id_orden=tic_responsable_orden.id_orden_trabajo "
        ...
        if(($_GET['checkTrab'])=='respon'):
                $table = 'tic_responsable_orden';
                $sJoin = "INNER JOIN tic_orden_trabajo ON tic_responsable_orden.id_orden_trabajo=tic_orden_trabajo.id_orden "
                        . "INNER JOIN mnt_estatus ON tic_orden_trabajo.estatus=mnt_estatus.id_estado "             
                        . "LEFT JOIN tic_asigna_cuadrilla ON tic_responsable_orden.id_orden_trabajo=tic_asigna_cuadrilla.id_ordenes "
                        . "LEFT JOIN tic_cuadrilla ON tic_asigna_cuadrilla.id_cuadrilla=tic_cuadrilla.id "
                        . "INNER JOIN tic_tipo_orden ON tic_orden_trabajo.id_tipo=tic_tipo_orden.id_tipo "
                        . "INNER JOIN dec_dependencia ON tic_orden_trabajo.dependencia=dec_dependencia.id_dependencia " 
//                        . "LEFT JOIN tic_ayudante_orden ON tic_responsable_orden.id_orden_trabajo=tic_ayudante_orden.id_orden_trabajo "
                        . "INNER JOIN dec_usuario ON tic_responsable_orden.id_responsable=dec_usuario.id_usuario";
        endif;
...
//                . "LEFT JOIN tic_asigna_cuadrilla ON tic_orden_trabajo.id_orden=tic_asigna_cuadrilla.id_ordenes "
//                . "LEFT JOIN tic_cuadrilla ON tic_asigna_cuadrilla.id_cuadrilla=tic_cuadrilla.id ";
//                . "LEFT JOIN tic_ayudante_orden ON tic_orden_trabajo.id_orden=tic_ayudante_orden.id_orden_trabajo "
//                . "LEFT JOIN dec_usuario ON tic_ayudante_orden.id_trabajador=dec_usuario.id_usuario";
//            endif;
        endif;
        $band =0;       
        if ($sWhere == ""):
            ...
            if (isset($_GET['tipo_orden']) AND $_GET['tipo_orden'] != ""):
                if ($band):
                    $sQuery = $sQuery . " AND tic_orden_trabajo.id_tipo = '$_GET[tipo_orden]' $sOrder $sLimit";
                else:
                    $sQuery = $sQuery . " WHERE tic_orden_trabajo.id_tipo in ('$_GET[tipo_orden]') $sOrder $sLimit";
                endif;
            else:
...
//            la ejecucion segun lo que se necesita.
            ...
            if (isset($_GET['tipo_orden']) AND $_GET['tipo_orden'] != ""):
                if ($band):
                    ...
                    $sQuery = $sQuery . " AND tic_orden_trabajo.id_tipo in ('$_GET[tipo_orden]') $sOrder $sLimit";
                endif;
            else:
                $sQuery = $sQuery . " $sOrder $sLimit";
            endif;
