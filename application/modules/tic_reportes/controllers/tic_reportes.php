...
class Tic_reportes extends MX_Controller
{
	function __construct() //constructor predeterminado del controlador
    {
        parent::__construct();
        $this->load->library('form_validation');
        $this->load->model('model_tic_reporte');
        ...
        $this->load->model('tic_tipo/model_tic_tipo_orden');
    }

      //Esta funcion se una para construir el json para el llenado del datatable en la vista de reportes
    function list_sol() {
        $results = $this->model_tic_reporte->get_list();//Va al modelo para tomar los datos para llenar el datatable
...
//            echo_pre ($this->model_tic_cuadrilla->es_responsable($this->session->userdata('user')['id_usuario']));
//            echo_pre ($this->session->userdata('user'));
//            echo_pre($_GET);
//            echo_pre($this->model_tic_cuadrilla->es_resp_no_jefe_cuad($this->session->userdata('user')['id_usuario']));
            ...
            $id_tipo = $this->model_tic_cuadrilla->es_resp_no_jefe_cuad($this->session->userdata('user')['id_usuario']);//en caso de que sea jefe de cuadrilla devuelve el id tipo de orden
            if(isset($id_tipo))://se evalua si existe este valor para devolver el menu de busquedas a la vista.
                ...
                $view['trabajadores']=$this->model_tic_ayudante->ayud_tipo_orden($id_tipo);
            ...
            $this->load->view('tic_reportes/tic_reporte_trabajador',$view);
...
//        die_pre($view['estatus']); 
        ...
        if(($_POST['menu'])== 'trab'):
            if (($_POST['trabajadores'])):
                $view['tabla'] = $this->model_tic_ayudante->consul_trabaja_sol($_POST['trabajadores'],$estatus,$_POST['result1'],$_POST['result2'],$band,$_POST['buscador'],$_POST['col_pdf'],$_POST['dir_span']);//construccion de la tabla    
                $view['trabajador'] = $this->model_user->get_user_cuadrilla($_POST['trabajadores']);
//            die_pre($view);
            else:
                $view['tabla'] = $this->model_tic_ayudante->consul_trabaja_sol('',$estatus,$_POST['result1'],$_POST['result2'],$band,$_POST['buscador'],$_POST['col_pdf'],$_POST['dir_span']);//construccion de la tabla
            endif;
        endif;
//        die_pre($view);
        if(($_POST['menu'])== 'respon'){
...
//            echo_pre($view);
        ...
        if(($_POST['menu'])== 'tipo'):
            if (($_POST['tipo_orden'])):
                $view['tabla'] = $this->model_tic_solicitudes->consul_orden_tipo($_POST['tipo_orden'],$estatus,$_POST['result1'],$_POST['result2'],$band,$_POST['buscador'],$_POST['menu'],$_POST['col_pdf'],$_POST['dir_span'],$_POST['menu']);
                $tipo = $this->model_tic_tipo_orden->devuelve_tipo($_POST['tipo_orden']);
...
//            echo_pre($view);
                ...
                if(isset($id_tipo)){//PARA evaluar si existe el id de la cuadrilla/tipo de orden cuando esta el usuario responsable de cuadrilla en sesion
                    $view['tabla'] = $this->model_tic_solicitudes->consul_orden_tipo($id_tipo,$estatus,$_POST['result1'],$_POST['result2'],$band,$_POST['buscador'],$_POST['menu'],$_POST['col_pdf'],$_POST['dir_span'],$_POST['menu']);
                }else{
                    $view['tabla'] = $this->model_tic_solicitudes->consul_orden_tipo('',$estatus,$_POST['result1'],$_POST['result2'],$band,$_POST['buscador'],$_POST['menu'],$_POST['col_pdf'],$_POST['dir_span'],$_POST['menu']);
                }
//                foreach ($view['tabla'] as $dat):
//                    $ayudantes[$dat['id_orden']] = $this->model_tic_ayudante->ayudantes_DeOrden($dat['id_orden']);
//                endforeach;
//                $view['ayudantes']=$ayudantes;
        ...
        if(($_POST['menu']) == ''){
            if(isset($id_tipo)){//PARA evaluar si existe el id de la cuadrilla/tipo de orden cuando esta el usuario responsable de cuadrilla en sesion
                $view['tabla'] = $this->model_tic_solicitudes->consul_orden_tipo($id_tipo,$estatus,$_POST['result1'],$_POST['result2'],$band,$_POST['buscador'],'',$_POST['col_pdf']);
            }else{
                $view['tabla'] = $this->model_tic_solicitudes->consul_orden_tipo('',$estatus,$_POST['result1'],$_POST['result2'],$band,$_POST['buscador'],'',$_POST['col_pdf']);
            }
...
//        die_pre($view);
         ...
         */
        $this->pdf->SetTitle("Reportes DTIC Solicitudes");
...
//            $this->dompdf->stream("asignaciones.pdf", array('Attachment' => 0));
    ...
    {//Solo si es usuario autoridad y/o Asistente de autoridad
        return ($this->session->userdata('user')['sys_rol'] == 'autoridad' || $this->session->userdata('user')['sys_rol'] == 'asist_autoridad' || $this->session->userdata('user')['sys_rol'] == 'jefe_tic');
    }
    ...
    {//Solo si es usuario "Director de Dependencia y/o Asistente de dependencia"
        return ($this->session->userdata('user')['sys_rol'] == 'asistente_dep_alm' || $this->session->userdata('user')['sys_rol'] == 'asistente_dep_tic'|| $this->session->userdata('user')['sys_rol'] == 'asistente_dep');
    }
